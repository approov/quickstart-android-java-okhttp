...
* A solid understanding of how to integrate Approov into your own app that uses Java and [OkHttp](https://square.github.io/okhttp/)
...
The folder `framework` has a `src/main/java/io/approov/framework/okhttp/ApproovService.java` file. This provides a wrapper around the Approov SDK itself to make it easier to use with `OkHttp`. Show the terminal in Android Studio by clicking on the `Terminal` tab, typically located at the bottom of the screen:
